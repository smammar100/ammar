// What the demo card actually copies, baked in from the real sources so the
// buttons work offline: the .tsx is this repo's ported registry component, the
// prompt is a snapshot of iconimate.app/api/prompt/acorn.
export const ACORN_CLI = "npx shadcn@latest add https://iconimate.app/r/acorn.json";

export const ACORN_CODE = `"use client";
...
export const ACORN_PROMPT = `Create the "Acorn" animated icon for Iconimate: a React component with
hand-tuned motion, drawn on the Phosphor 256 grid and tuned to read at 24px.

GLYPH
Phosphor "acorn" on the 256 grid. Subpaths, which must recompose to the
